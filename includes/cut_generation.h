...
class CutStorage{
public:
    CutStorage(vector<Scalar> &obj_initial_storage, vector<Vec> &grad_initial_storage, vector<Vec> &x_initial_storage);
    ...
    friend Vec  master_milp(CutStorage &StoragePool, int &N, Scalar &M, int &kappa, int &current_iter);
//private:
